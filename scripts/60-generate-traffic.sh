...
echo "     # NOTE: If 'az monitor' fails to load in your CLI environment, use ARM directly:"
echo "     az rest --method get --uri \"https://management.azure.com/subscriptions/$AZURE_SUBSCRIPTION_ID/resourceGroups/$OCTOPETS_RG_NAME/providers/Microsoft.App/containerApps/octopetsapi/providers/microsoft.insights/metrics?api-version=2018-01-01&metricnames=WorkingSetBytes&interval=PT1M\""
echo ""
echo "  2. Check for alerts:"
echo "     az rest --method get --uri \"https://management.azure.com/subscriptions/$AZURE_SUBSCRIPTION_ID/resourceGroups/$OCTOPETS_RG_NAME/providers/Microsoft.Insights/metricAlerts?api-version=2018-03-01\" --query \"value[].{name:name, enabled:properties.enabled, severity:properties.severity}\" -o table"
echo ""
